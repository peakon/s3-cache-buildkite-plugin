...
describe('S3CacheAdapter', () => {
  ...
  describe('when gzip=false', () => {
    let adapter;
    beforeEach(() => {
      adapter = new S3CacheAdapter({
        bucket: 'buildkite-cache',
        keyPrefix: 'any',
        gzip: false,
      });
    });
  ...
  describe('when saving with gzip=true and restoring with gzip=false', () => {
    it('detects gzipped archives and automatically restores them', async () => {
      const saveAdapter = new S3CacheAdapter({
        bucket: 'buildkite-cache',
        keyPrefix: 'any',
        gzip: true,
      });
      await saveAdapter.saveItem('dir', ['.tmp']);

      cleanTempDir();
      const restoreAdapter = new S3CacheAdapter({
        bucket: 'buildkite-cache',
        keyPrefix: 'any',
        gzip: false,
      });
      await restoreAdapter.restoreItem('dir');
      expect(pathExists('.tmp/testfile.txt')).toBe(true);
      expect(pathExists('.tmp/testfile2.txt')).toBe(true);
      expect(pathExists('.tmp/subfolder/testfile.txt')).toBe(true);
    });
