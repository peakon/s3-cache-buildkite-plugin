...
const cli = meow(
  `
    Usage
      $ node lib --bucket=bucketname --action=save|restore --prefix=$pipelineName --stepExitCode=exitCode
  `,
  {
    description: false,
    flags: {
...
(async () => {
  ...
  if (s3CachePlugin) {
    ...
    const { action, bucket, prefix, stepExitCode } = cli.flags;
    ...
    const cacheLoader = new CacheLoader(new S3CacheAdapter(bucket, prefix));
